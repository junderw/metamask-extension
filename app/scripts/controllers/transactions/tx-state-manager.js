...
export default class TransactionStateManager extends EventEmitter {
  ...
  addTransaction(txMeta) {
    ...
    // persistence limit and then if it is removes the oldest confirmed or
    // rejected tx. Pending or unapproved transactions will not be removed by
    // this operation.
    //
    ...
    // some point in the future, this persistence limit can be lifted. When we
    // do that I think we should figure out a better storage solution for
  ...
  getTransactions({
    ...
    const filteredTransactions = sortBy(
      pickBy(transactionsToFilter, (transaction) => {
        ...
        let matchesCriteria = filterToCurrentNetwork
          ? transactionMatchesNetwork(transaction, chainId, network)
          : true;
        // iterate over the predicateMethods keys to check if the transaction
        // matches the searchCriteria
        Object.keys(predicateMethods).forEach((key) => {
          // We don't need to check any additional predicate methods if we have
          // already identified the transaction doesn't match the criteria.
          if (matchesCriteria) {
            // predicate keys can be either in txParams or in the base object.
            if (key in transaction.txParams) {
              matchesCriteria = predicateMethods[key](
                transaction.txParams[key],
              );
            } else {
              matchesCriteria = predicateMethods[key](transaction[key]);
            }
          }
        });
